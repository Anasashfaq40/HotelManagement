...
const Reception = () => {
  ...
  // Handle Edit Staff Name
  const handleEdit = (booking) => {
    setCurrentBooking(booking);
    setStaffName(booking.staffName);
  ...
  const handleStaffNameSubmit = async () => {
    try {
      const response = await axios.put(
        `http://localhost:5000/api/bookingdata/${currentBooking._id}/staff`,
        { staffName }
      );
      ...
      alert("Staff name updated successfully!");
    } catch (error) {
      console.error("Error updating staff name:", error);
      alert("Failed to update staff name. Please try again.");
    }
  };
